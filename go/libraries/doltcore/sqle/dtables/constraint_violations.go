...
func NewConstraintViolationsTable(ctx *sql.Context, tblName string, root *doltdb.RootValue, rs RootSetter) (sql.Table, error) {
	...
	infoType, err := typeinfo.FromSqlType(
		sql.MustCreateStringWithDefaults(sqltypes.VarChar, varcharMaxByteLength/sql.Collation_Default.CharacterSet().MaxLength()))
	if err != nil {
		return nil, err
	}
	...
	infoCol, err := schema.NewColumnWithTypeInfo("violation_info", schema.DoltConstraintViolationsInfoTag, infoType, false, "", false, "")
	if err != nil {
		return nil, err
	}
